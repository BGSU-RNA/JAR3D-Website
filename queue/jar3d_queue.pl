...
MAIN:
{
    ...
    while (! $TERM) {
       ...
       if ( scalar(@queries) > 0 ) {
            ...
            my $work = "ulimit -t $TIMEOUT; java -Xmx500m -jar $RealBin/webJAR3D_server.jar $config{il_models} $config{hl_models} $query_id $config{db_user_name} $config{db_password} $config{db_database}";
