...
MAIN:
{
    ...
    while (! $TERM) {
       ...
       if ( scalar(@queries) > 0 ) {
            ...
            my $work = "ulimit -t $TIMEOUT; java -Xmx500m -jar $RealBin/webJAR3D_server.jar /Users/api/Models/IL/1.13/lib/all.txt /Users/api/Models/HL/1.13/lib/all.txt $query_id $config{db_user_name} $config{db_password} $config{db_database}";
