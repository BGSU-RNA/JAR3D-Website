...
MAIN:
{
    ...
    while (! $TERM) {
       ...
       if ( scalar(@queries) > 0 ) {
            ...
            my $work = "ulimit -t $TIMEOUT; java  -jar $RealBin/webJAR3D_server.jar /Users/anton/Dropbox/BGSURNA/Motifs/lib/ $query_id $config{db_user_name} $config{db_password} $config{db_database}";
