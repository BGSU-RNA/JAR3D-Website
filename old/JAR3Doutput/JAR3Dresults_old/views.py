...
from JAR3Doutput.JAR3Dresults.models import Query_info
from JAR3Doutput.JAR3Dresults.models import Query_sequences
from JAR3Doutput.JAR3Dresults.models import Query_loop_positions
from JAR3Doutput.JAR3Dresults.models import Results_by_loop
from JAR3Doutput.JAR3Dresults.models import Results_by_loop_instance
from JAR3Doutput.JAR3Dresults.models import Loop_query_info
from JAR3Doutput.JAR3Dresults.models import Correspondence_results

from JAR3Doutput import my_queue
...
def home(request, uuid=None):
    ...
    if uuid:
        ...
        if q:
            return render('JAR3Doutput/base_homepage.html',
...
def result(request, uuid):
    ...
    if q:
        ...
        return render('JAR3Doutput/base_result_not_found.html',
                                  {'query_id': uuid},
                                  context_instance=RequestContext(request))
    ...
    if q.status == 1:
        ...
        return render('JAR3Doutput/base_result_done.html',
                                  {'query_info': q, 'num': results.input_stats,
                                   'results': zippedResults, 'compress': False},
                                  context_instance=RequestContext(request))
    elif q.status == 0 or q.status == 2:
        q.formatted_input = make_input_alignment(q.parsed_input, q.query_type)
        return render('JAR3Doutput/base_result_pending.html',
                                  {'query_info': q, 'num': results.input_stats},
                                  context_instance=RequestContext(request))
    else:
        return render('JAR3Doutput/base_result_failed.html',
                                  {'query_info': q, 'num': results.input_stats},
                                  context_instance=RequestContext(request))
...
def single_result(request, uuid, loopid, motifgroup):
    ...
    if uuid in exclude_list:
        ...
        return render('JAR3Doutput/base_result_loop_failed.html',
                                  {'query_info': q,
    ...
    if q:
        q = q[0]  # We are interested only in the first one
        if q.status == 0:
            return render('JAR3Doutput/base_result_loop_pending.html',
                                      {'query_info': q,
    ...
    if ShowResult:
        ...
        return render('JAR3Doutput/base_result_loop_done.html',
...
class JAR3DValidator():
    ...
    def validate(self, request):
        ...
        try:
            query_info.save()
            my_queue.score({'id': query_id, 'version': version})
        except Exception:
...
def alignsequencesandinstancesfromtext(MotifCorrespondenceText, SequenceCorrespondenceText):
    ...
    InstanceToGroup, InstanceToPDB, InstanceToSequence, GroupToModel, ModelToColumn, NotSequenceToModel, \
        HasName = readcorrespondencesfromtext(MotifCorrespondenceText)[:7]
    NotInstanceToGroup, NotInstanceToPDB, NotInstanceToSequence, NotGroupToModel, NotModelToColumn, \
        SequenceToModel = readcorrespondencesfromtext(SequenceCorrespondenceText)[:6]
